    ...
    <section ref={sectionRef} className="relative min-h-[90vh] sm:min-h-screen overflow-hidden flex flex-col justify-end">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden">
        <img
          src={heroImage}
          alt={t.nav.brandName}
          className="w-full h-full object-cover object-center sm:scale-105 filter brightness-[0.98] contrast-[1.04]"
        />
      </div>

      {/* Multi-layered responsive gradient overlay */}
      {/* Bottom up gradient: gentle transition into the page canvas */}
      <div className="absolute inset-0 z-1 bg-gradient-to-t from-canvas via-canvas/75 via-30% to-transparent pointer-events-none" />
      ...
      >
        <div className="lg:max-w-3xl text-start">
          ...
          <div className="p-5 xs:p-7 sm:p-0 rounded-3xl sm:rounded-none bg-surface/90 sm:bg-transparent backdrop-blur-2xl sm:backdrop-blur-none border border-muted-border/40 sm:border-0 shadow-2xl shadow-black/10 sm:shadow-none transition-all">
            {/* Badge */}
            <div
              className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full brand-badge text-[11px] sm:text-xs font-bold w-fit mb-3.5 sm:mb-7 hero-reveal shadow-lg shadow-accent/15 border border-accent/40 max-w-full"
            ...
            <h1 className="text-2xl xs:text-3xl sm:text-5xl lg:text-6xl xl:text-7xl font-black leading-[1.25] sm:leading-[1.15] tracking-tight">
              <span className="block text-heading hero-reveal drop-shadow-xs" style={{ animationDelay: '300ms' }}>
                {t.hero.titleLine1}
              </span>
              <span
                className="block brand-gradient-text hero-reveal mt-1 sm:mt-2 drop-shadow-xs"
            ...
            >
              <button
                onClick={() => onExplore()}
                className="w-full sm:w-auto brand-btn-primary font-black px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl inline-flex items-center justify-center gap-2.5 hover:scale-105 active:scale-95 transition-all duration-300 cursor-pointer shadow-lg shadow-accent/25 text-xs xs:text-sm sm:text-base"
              >
              ...
              <button
                onClick={onBook}
                className="w-full sm:w-auto brand-btn-secondary font-bold px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl inline-flex items-center justify-center gap-2 hover:scale-105 active:scale-95 transition-all duration-300 cursor-pointer text-xs xs:text-sm sm:text-base"
              >
